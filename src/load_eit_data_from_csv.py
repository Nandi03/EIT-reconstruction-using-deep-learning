...
class DataLoader:
    ...
    Attributes:
        ...
        baseline (np.ndarray or None): Baseline voltage readings for normalization.
    ...
    def load_data(self, single_touch_file, multi_touch_file, three_d=False, force=False, randforce=False):
        ...
        self._initialize_baseline(single_touch_data, three_d)
    ...
    def _initialize_baseline(self, data, three_d):
        """Set the baseline from the first sample."""
        if not three_d:
            self.baseline = data.iloc[0, 2: 2 + self.pairs_electrodes]
        else:
            self.baseline = data.iloc[0, 3: 3 + self.pairs_electrodes]
    ...
    def _map_coordinates(self, data, three_d):
        ...
        for i, row in data.iterrows():
            ...
            if coord not in self.coordinate_to_index:
                ...
                index += 1
        self.num_classes = len(self.coordinate_to_index)
